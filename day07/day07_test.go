...
func TestCalculateThrustWithFeedbackLoop(t *testing.T) {
	for _, testCase := range testCases2 {
		thrust := CalculateThrustWithFeedbackLoop(testCase.startingCode, testCase.phaseSequence, 0)
		if thrust != testCase.expectedThrust {
			t.Errorf("Error, expected %v got %v", testCase.expectedThrust, thrust)
		}
	}
